...
Elm.Html.Animation.Core.make = function (_elm) {
   ...
   var step = F4(function (frame,prev,current,dt) {
      ...
      F2(function (x,acc) {
         ...
         if (_p96.ctor === "Nothing") {
               ...
               return A2($Basics._op["++"],acc,_U.list([A3(stepProp,x,_p96._0,A2(applyStep,current,dt))]));
            }
      }),
      _U.list([]),
      frame.target);
      ...
      if (_U.eq(dt,0)) return A3($continue,model,elapsed,start); else if (A2(done,elapsed,current)) {
               ...
               return {ctor: "_Tuple2"
                      ...
                      ,_1: $Effects.tick(Tick)};
                          ...
                          ,anim: A3(mapTo,0,function (a) {    return A4(step,a,model.previous,elapsed,dt);},model.anim)})
                          ,_1: $Effects.tick(Tick)};
   });
...
Elm.Main.make = function (_elm) {
   ...
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var iconCache = _U.list(["pencil","at","camera","bell","comment","bolt","ban","code"]);
   var icons = A2($List.take,5,iconCache);
   var fanAngle = 0.11;
   var createSubmenu = F3(function (icon,total,i) {
      var adjustment = 0.5 - ($Basics.toFloat(total) - 1) / 2.0 * fanAngle;
      var angle = $Basics.toFloat(i) * fanAngle + adjustment;
      return {icon: icon
             ,style: $Html$Animation.init(_U.list([A2($Html$Animation$Properties.Rotate,angle,$Html$Animation$Properties.Turn)
                                                  ,A2($Html$Animation$Properties.TranslateY,0,$Html$Animation$Properties.Px)
                                                  ,A2($Html$Animation$Properties.Rotate,-1 * angle,$Html$Animation$Properties.Turn)]))};
   });
   var init = {open: false
              ,submenus: A2($List.indexedMap,F2(function (i,icon) {    return A3(createSubmenu,icon,$List.length(icons),i);}),icons)
              ,style: $Html$Animation.init(_U.list([A2($Html$Animation$Properties.Rotate,-0.125,$Html$Animation$Properties.Turn)]))
              ,message: $Maybe.Nothing};
   var viewMessage = function (msg) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("message"),$Html$Attributes.style($Html$Animation.render(msg.style))]),
      _U.list([$Html.text(msg.label)]));
   };
   var OnMessage = function (a) {    return {ctor: "OnMessage",_0: a};};
   var OnSubmenu = F2(function (a,b) {    return {ctor: "OnSubmenu",_0: a,_1: b};});
   var OnMenu = function (a) {    return {ctor: "OnMenu",_0: a};};
   var Animate = function (a) {    return {ctor: "Animate",_0: a};};
   var onMenu = A3($Html$Animation.forwardTo,
   function (a) {
      return Animate(OnMenu(a));
   },
   function (_) {
      return _.style;
   },
   F2(function (w,style) {    return _U.update(w,{style: style});}));
   var onAllSubmenus = A3($Html$Animation.forwardToAll,
   F2(function (i,a) {    return Animate(A2(OnSubmenu,i,a));}),
   function (_) {
      return _.style;
   },
   F2(function (w,style) {    return _U.update(w,{style: style});}));
   var onSubmenu = A3($Html$Animation.forwardToIndex,
   F2(function (i,a) {    return Animate(A2(OnSubmenu,i,a));}),
   function (_) {
      return _.style;
   },
   ...
   var update = F2(function (action,model) {
      ...
      {case "Toggle": if (model.open) {
                 ...
                 return {ctor: "_Tuple2",_0: _U.update(newMenu,{submenus: submenus,open: false}),_1: $Effects.batch(_U.list([submenuFx,menuFx]))};
              } else {
                 var _p3 = A2(onMenu,
                 model,
                 A2($Html$Animation.props,
                 ...
                 return {ctor: "_Tuple2",_0: _U.update(newMenu,{submenus: submenus,open: true}),_1: $Effects.batch(_U.list([submenuFx,menuFx]))};
              }
         case "ShowMessage": var message = {label: _p0._0
                                           ,style: $Html$Animation.init(_U.list([$Html$Animation$Properties.Display($Html$Animation$Properties.Block)
                                                                                ,$Html$Animation$Properties.Opacity(0)]))};
           var _p5 = A2($Html$Animation.on,
           message.style,
           A2($Html$Animation.set,
           _U.list([$Html$Animation$Properties.Display($Html$Animation$Properties.None)]),
           $Html$Animation.andThen(A2($Html$Animation.props,
           _U.list([$Html$Animation$Properties.Opacity($Html$Animation.to(0))]),
           $Html$Animation.andThen(A2($Html$Animation.props,_U.list([$Html$Animation$Properties.Opacity($Html$Animation.to(1))]),$Html$Animation.animate))))));
           var msgStyle = _p5._0;
           var fx = _p5._1;
           var msg = _U.update(message,{style: msgStyle});
           return {ctor: "_Tuple2",_0: _U.update(model,{message: $Maybe.Just(msg)}),_1: A2($Effects.map,function (a) {    return Animate(OnMessage(a));},fx)};
         default: var _p6 = _p0._0;
           switch (_p6.ctor)
           {case "OnMenu": return A2(onMenu,model,_p6._0);
              case "OnSubmenu": var _p7 = A3(onSubmenu,_p6._0,model.submenus,_p6._1);
                var submenus = _p7._0;
                var fx = _p7._1;
                return {ctor: "_Tuple2",_0: _U.update(model,{submenus: submenus}),_1: fx};
              default: var _p8 = model.message;
                if (_p8.ctor === "Nothing") {
                      return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
                   } else {
                      var _p10 = _p8._0;
                      var _p9 = A2($Html$Animation.update,_p6._0,_p10.style);
                      var msgStyle = _p9._0;
                      var fx = _p9._1;
                      var newMsg = _U.update(_p10,{style: msgStyle});
                      return {ctor: "_Tuple2"
                             ,_0: _U.update(model,{message: $Maybe.Just(newMsg)})
                             ,_1: A2($Effects.map,function (a) {    return Animate(OnMessage(a));},fx)};
                   }}}
   });
   var ShowMessage = function (a) {    return {ctor: "ShowMessage",_0: a};};
   var viewSubmenu = F2(function (address,submenu) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("child-button")
              ,$Html$Attributes.style($Html$Animation.render(submenu.style))
              ,A2($Html$Events.onClick,address,ShowMessage(submenu.icon))]),
      _U.list([A2($Html.i,_U.list([$Html$Attributes.$class(A2($Basics._op["++"],"fa  fa-lg fa-",submenu.icon))]),_U.list([]))]));
   });
   var Toggle = {ctor: "Toggle"};
   var view = F2(function (address,model) {
      var submenus = A2($List.map,viewSubmenu(address),model.submenus);
      var message = A2($Maybe.withDefault,A2($Html.div,_U.list([]),_U.list([])),A2($Maybe.map,viewMessage,model.message));
      var icon = A2($Html.i,_U.list([$Html$Attributes.$class("fa fa-close fa-3x"),$Html$Attributes.style($Html$Animation.render(model.style))]),_U.list([]));
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("main-button"),A2($Html$Events.onClick,address,Toggle)]),
      A2($List._op["::"],icon,A2($List._op["::"],message,submenus)));
   });
   var app = $StartApp.start({init: {ctor: "_Tuple2",_0: init,_1: $Effects.none},update: update,view: view,inputs: _U.list([])});
   var main = app.html;
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   var Message = F2(function (a,b) {    return {label: a,style: b};});
   var Submenu = F2(function (a,b) {    return {style: a,icon: b};});
   var Model = F4(function (a,b,c,d) {    return {submenus: a,style: b,open: c,message: d};});
